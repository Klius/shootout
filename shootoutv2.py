...
ai = {
    ...
    'default': {'shoot': False, 'cover': False},
    'rare': {'shoot': True, 'cover': False}
}
...
def update():
    ...
    if player['hp'] <= 0 or player2['hp'] <= 0:
        ...
        if key == curses.KEY_DOWN or key == "KEY_C2":
            player['cover'] = True
...
def draw(win):
    ...
    if gameover == False:
        ...
        win.addstr(sky+"\n\n\n")
        if player['bullet']['moving']:
            field = field[:player['bullet']['pos']] + \
                '▸' + field[player['bullet']['pos'] + 1:]
        if player2['bullet']['moving']:
            field = field[:player2['bullet']['pos']] + \
                '◂' + field[player2['bullet']['pos'] + 1:]
